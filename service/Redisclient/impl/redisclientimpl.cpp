    ...
    if( state == RedisClientImpl::Subscribed )
    {
        ...
        if( result.size() == 3 )
        {
            ...
            if( cmd == "message" )
            {
                ...
                {
                    strand.post(boost::bind(handlerIt->second.second, value.toByteArray(), queueName.toString()));
                }
            }
        ...
        {
            // Sub client result of running command
            ...
            else
            {
                ...
                ss << "[RedisSubClient] unexpected message: "
                   <<  v.inspect();
