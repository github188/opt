...
{
/**
pthread_exit()调用偶现导致异常
#0  0x76eac650 in raise () from /lib/libc.so.0
#1  0x76ea6b20 in abort () from /lib/libc.so.0
#2  0x76f299f8 in pthread_cancel_init () from /lib/libpthread.so.0
#3  0x76f29b0c in _Unwind_ForcedUnwind () from /lib/libpthread.so.0
#4  0x76f358e4 in __pthread_unwind () from /lib/libpthread.so.0
#5  0x76f30274 in pthread_exit () from /lib/libpthread.so.0
#6  0x00045834 in st_thread_exit (retval=0x0) at stt.c:82
#7  0x0007416c in server (ptr=0x0) at client/client.c:1220
#8  0x76f2ef9c in start_thread () from /lib/libpthread.so.0
#9  0x76eabb58 in clone () from /lib/libc.so.0
**/
    //pthread_exit(retval);
    return;
}
                             ...
                             int joinable, int stack_size)
{
    ...
    if(stack_size > 0)
    {
        ...
        if(ret) fprintf(stderr, "set pthread stack size %d failed with error %d!\n", stk_size, errno);
    }

    if(pthread_create(&thread, &thread_attr, start, arg)==0 && joinable==0){
...
st_netfd_t _st_netfd_new(int osfd, int nonblock, int is_socket)
{
    ...
    fd = calloc(1, sizeof(struct _st_netfd));
    if (!fd)
        return NULL;
                      ...
                      st_utime_t timeout)
{
    ...
    while (connect(fd->osfd, addr, addrlen) < 0)
    {
        if (errno != EINTR)
        {
            ...
            {
                //errno = err;
                printf("st_connect[nfd %p(fd=%d)] failed: %s\n", fd, fd->osfd, strerror(errno));
                return -1;
            }
            break;
        }
